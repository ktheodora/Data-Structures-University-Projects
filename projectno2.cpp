#include <stdlib.h> //óõìðåñéëáìâÜíïõìå ôçí stdlib.h ãéá íá ÷ñçóéìïðïéÞóïõìå ôéò óõíáñôÞóåéò malloc êáé free óôçí õëïðïßçóç ôïõ äÝíäñïõ
#include <iostream>
#include <string>
using namespace std;
//ïñßæïõìå ôéò search1 êáé delete1 ÷ùñßò íá ãñÜøïõìå ôçí õëïðïßçóç ôïõò, 
//äéüôé óôçí óõíÝ÷åéá êáëïýíôáé áíáäñïìéêÜ ìåôáîý ôïõò
void search1(struct treenode *t, int data);
void delete1(struct treenode *t);
struct treenode //ïñéóìüò êïíóôñÜêôïñá treenode
{
	int value;//èá ðåñéâáëìÜíåé ôï êëåéäß ôïõ êüìâïõ
	struct treenode *l; //äåßêôçò áñéóôåñïý êëåéäéïý
	struct treenode *r;//äåßêôçò äåîéïý êëåéäéïý
}
//áñ÷éêïðïßçóç ôçò ñßæáò êáé ôùí ðñïóùñéíþí äåéêôþí 
	...
	while (booly) {
		...
		if (ch == 1) {
			...
			while ((arrivals[i] < 1) && (arrivals[i] > 24)) {
				...
				cin >> arrivals[i];
			} //äåí âãáßíù áðï ôçí åðáíÜëçøç áí äåí âåâáéùèþ üôé Ý÷åé äïèåß ðñáãìáôéêÞ þñá
			...
			for (int j = 0; j < i; j++) {
				if ((arrivals[j] > hour1) && (arrivals[j] <= hour2)) { //áí ç þñá áöéîçò åéíáé ìåãáëõôåñç ôçò åíáñîçò ôïõ ÷ñïíéêïõ äéáôçìáôïò
																	   //êáé ìéêñïôåñç Þ ßóç ôïõ ôåëïõò ôïõ ÷ñïí. äéáóôÞìáôïò
					cout << "\n" << codes[j];
					check = 1;
				}
			}
